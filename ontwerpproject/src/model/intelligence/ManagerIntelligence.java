...
public class ManagerIntelligence extends Intelligence {
	...
	public ManagerIntelligence(Component comp, Model mod, Connection conn) throws ClosedException {
		super(comp, mod, conn);
			...
			try {
				...
				if (r.next()) {
					if (newin[i] > r.getInt(1)) {
						errorMail(cols[i] + " exceeded the critical value in "
								+ comp.getTableName(), "critical value");
					}
				}
			} catch (SQLException e) {
				//databaseError(e);
